...
import com.doulevo.userapi.exception.GeneralRuntimeException;
import com.doulevo.userapi.util.function.Appender;
...
public class StringUtils {
  ...
  private static final Pattern listElemDelim = Pattern.compile("\\,\\s");
  ...
  public static String generateMessage(String entity, Map<String, String> searchParams) {
    return String.format(
        "%s was not found for parameters  %s",
        org.apache.commons.lang3.StringUtils.capitalize(entity), searchParams);
  }
  ...
  public static String replaceString(String mainString, String oldString, String newString) {
    ...
    if (ObjectUtils.isEmpty(oldString)) {
      return mainString;
    }
    if (newString == null) {
      newString = "";
  ...
  public static List<String> split(String str, String delim, int limit) {
    ...
    //  2018-10-18: switched to String.split because is better optimized for single-char case
    // than Pattern.compile
    // if (delim != null) st = Pattern.compile(delim).split(str, limit);
    if (delim != null) {
      st = str.split(delim, limit);
    } else {
      st = str.split("\\s", limit); //  2018-10-18: fixed missing limit in this case (stock bug)
    }

    if (st != null && st.length > 0) {
      //  NOTE: Can't safely do Arrays.asList from here, because throws exception if tries to
      // add items,
      // and can't assume a caller won't try (old interface); but assume few callers do that and set
      // capacity tight
      splitList =
          new ArrayList<String>(Arrays.asList(st)); //  switched to ArrayList and copy constructor
  ...
  public static List<String> quoteStrList(List<String> list) {
    ...
    list = new ArrayList<String>(list.size()); //  switched to ArrayList
   ...
   * @param delim the delimiter character(s) to join on (null will split on whitespace)
   * @param trim Trim whitespace off fields
   * @param pairsSeparator in case you use not encoded name/value pairs strings and want to replace
   *     "=" to avoid clashes with parameters values in a not encoded URL, default to "="
   * @return a Map of name/value pairs
   */
  public static Map<String, String> strToMap(
      String str, String delim, boolean trim, String pairsSeparator) {
    if (str == null) {
      return null;
    }
  ...
  public static List<String> split(String str, String delim) {
    ...
    if (st.hasMoreTokens()) {
      splitList =
          new ArrayList<String>(); //  switched to ArrayList (default capacity is usually good here)
  ...
  /**
   * Reads a String version of a Map (should contain only strings) and creates a new Map. Partial
   * Map elements are skipped: <code>{foo=fooValue, bar=}</code> will contain only the foo element.
   *
  ...
  public static Map<String, String> toMap(String s) {
    ...
    if (s.startsWith("{") && s.endsWith("}")) {
      ...
      //  Pre-compiled delim Pattern, because more than 2 chars (not optimized by
      // String.split)
  ...
  public static List<String> toList(String s) {
    // List<String> newList = new LinkedList<String>(); //  switched to ArrayList (below)
    if (s.startsWith("[") && s.endsWith("]")) {
      s = s.substring(1, s.length() - 1);
      //  Pre-compiled delim Pattern, because more than 2 chars (not optimized by
      // String.split)
      ...
      List<String> newList =
          new ArrayList<String>(
              Arrays.asList(entries)); //  switched to ArrayList and copy constructor
      // for (String entry: entries) {
      //    newList.add(entry);
      // }
      return newList;
  ...
  public static <K, V> Map<K, V> createMap(List<K> keys, List<V> values) {
    if (keys == null || values == null || keys.size() != values.size()) {
      throw new IllegalArgumentException(
          "Keys and Values cannot be null and must be the same size");
    }
  ...
  /**
   * Ensures the string does not pass the given length and returns the first lengthLimit characters
   * if longer, with extra suffix to add if passes the length (e.g., "...").
   *
   * <p>Added 2018-09-17.
   */
  ...
  /** Checks if the given string/StringBuilder starts with the given character. */
  public static boolean startsWith(CharSequence str, char suffix) {
    return (str.length() > 0) && (str.charAt(0) == suffix);
  }

  /** Checks if the given string/StringBuilder starts with the given prefix. */
  public static boolean startsWith(CharSequence str, CharSequence prefix) {
    return StringUtils.startsWith(str, prefix);
  }

  /** Checks if the given String/StringBuilder ends with the given suffix. */
  public static boolean endsWith(CharSequence str, CharSequence suffix) {
    return StringUtils.endsWith(str, suffix);
  }

  /**
   *  Checks if the given object's {@link Object#toString()) value ends with the given character.
  ...
  /** Checks if the given String/StringBuilder ends with the given character. */
  public static boolean endsWith(CharSequence str, char suffix) {
    return (str.length() > 0) && (str.charAt(str.length() - 1) == suffix);
  }

  /**
   * Returns the given object as a CharSequence as-if if it is one, otherwise returns the object's
   * {@link Object#toString()} value.
   *
   * <p>NOTE: May be used on {@link Appendable} instances that are expected to hold strings.
   */
  ...
  /**
   *  Checks if the given object's {@link Object#toString()) value ends with the given suffix.
   * CharSequence-optimized.
   */
  ...
  /**
   * Append the string to the buffer after removing the given suffixToRemove from the end of the
   * string if present. NOTE: Only removes one occurrence of the character.
   */
  public static void appendWithoutSuffix(Appendable buffer, CharSequence str, char suffixToRemove)
      throws IOException {
   ...
   * string if present.
   */
  public static void appendWithoutSuffix(
      Appendable buffer, CharSequence str, CharSequence suffixToRemove) throws IOException {
    if (endsWith(str, suffixToRemove)) {
  ...
  public static String maskLeft(CharSequence str, int maskLength) {
    return maskLeft(str, maskLength, '*');
  }
  /**
   * Replaces the first characters of the string by the given character.
   *
   * <p>If maskLength is greater than zero, it determines the number of characters to mask from the
   * left; if negative, the number of characters to leave unmasked from the right; if zero, returns
   * the original string.
   *
   * <p>If maskLength is greater than zero and the string is less than maskLength, returns a string
   * of maskLength masked characters; if maskLength is negative and the string is less than the
   * absolute value of maskLength, returns the original string.
   */
  ...
  /**
   * Replaces the last characters of the string by the given character.
   *
   * <p>If maskLength is greater than zero, it determines the number of characters to mask from the
   * right; if negative, the number of characters to leave unmasked from the left; if zero, returns
   * the original string.
   *
   * <p>If maskLength is greater than zero and the string is less than maskLength, returns a string
   * of maskLength masked characters; if maskLength is negative and the string is less than the
  ...
  /** Checks if the two strings match, ignoring any number of mask characters to the left. */
  public static boolean matchesMaskedLeft(
      CharSequence plainStr, CharSequence maskedStr, char[] maskChars) {
  ...
  /** Unsafe version of Apache commons ArrayUtils.indexOf (unsafe). */
  ...
  /** Checks if two strings of same size have same characters between the given indices (unsafe). */
  private static boolean substringsEqual(
      CharSequence str1, CharSequence str2, int startIndex, int endIndex) {
  ...
  /** Checks if the two strings match, ignoring any number of mask characters to the left. */
  public static boolean matchesMaskedLeft(
      CharSequence plainStr, CharSequence maskedStr, char maskChar) {
    if (maskedStr == null || plainStr == null || maskedStr.length() != plainStr.length()) {
      return false;
    }
    int i = 0;
  ...
  /** Checks if the two strings match, ignoring any number of mask characters to the right. */
  public static boolean matchesMaskedRight(
      CharSequence plainStr, CharSequence maskedStr, char[] maskChars) {
    if (maskedStr == null || plainStr == null || maskedStr.length() != plainStr.length()) {
      return false;
    }
  ...
  /**
   * Checks if the two strings match, ignoring any mask characters at any place in the masked
   * string.
   */
  public static boolean matchesMaskedAny(
      CharSequence plainStr, CharSequence maskedStr, char[] maskChars) {
    if (maskedStr == null || plainStr == null || maskedStr.length() != plainStr.length()) {
      return false;
    }
  ...
  /**
   * A super-lightweight object to wrap a String object. Mainly used with FTL templates to avoid the
   * general HTML auto-encoding that is now done through the Screen Widget.
   */
